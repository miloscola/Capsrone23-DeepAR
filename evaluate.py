...
def evaluate(model, loss_fn, test_loader, params, plot_num, sample=True):
    ...
    model.eval()
    with torch.no_grad():
        # Randomly select a batch for plotting
        ...
        for i, (test_batch, id_batch, v, labels) in enumerate(tqdm(test_loader)):
            ...
            # Initialize tensors for predicted mean and standard deviation
            ...
            # If this is the plot_batch, create plots for visualization
            if i == plot_batch:
                if sample:
                    ...
                    sample_metrics = utils.get_metrics(sample_mu, labels, params.test_predict_start,
                                                       relative=params.relative_metrics)
                # Select 10 from samples with the highest error and 10 from the rest
                ...
                if batch_size < 100:  # make sure there are enough unique samples to choose the top 10 from
                ...
                if batch_size < 12:  # make sure there are enough unique samples to choose the bottom 90 from
